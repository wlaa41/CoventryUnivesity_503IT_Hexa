# BSc Computing Science Project — Professional Minutes of Meetings

**Coventry University, Dagenham, London**

## Meeting 1 of 13: Project Initiation and Team Formation

> **Team:** Sijan Bhandari · Johns Jimee · Abdhullah Badhurdeen · Aditya Bhandari · Sandesh Shah · Nabin Sunar

---

## Administrative Information

| Field               | Detail                                                      |
| ------------------- | ----------------------------------------------------------- |
| Meeting Number      | Meeting 1                                                   |
| Meeting Title       | Project Initiation and Team Formation                       |
| Date                | 12 May 2026                                                 |
| Time                | 10:00 am – 12:00 pm                                         |
| Location            | University Classroom — Coventry University Dagenham, London |
| Chairperson         | Sijan Bhandari                                              |
| Minutes Prepared By | Johns Jimee                                                 |
| Programme           | BSc Computing Science                                       |
| Institution         | Coventry University Dagenham, London                        |
| Module              | BSc Project / Dissertation                                  |
| Classification      | Confidential — Academic Use Only                            |

---

## Attendance Register

| Name                 | Role / Designation              | Programme             | Attendance |
| -------------------- | ------------------------------- | --------------------- | ---------- |
| Sijan Bhandari       | Project Lead / Chairperson      | BSc Computing Science | Present    |
| Johns Jimee          | Secretary / Technical Analyst / QA Engineer | BSc Computing Science | Present    |
| Abdhullah Badhurdeen | Lead Developer                  | BSc Computing Science | Present    |
| Aditya Bhandari      | Systems Architect / Planner     | BSc Computing Science | Present    |
| Sandesh Shah         | Research Associate              | BSc Computing Science | Present    |
| Nabin Sunar          | Implementation Support          | BSc Computing Science | Present    |

_All team members were present at this meeting._

---

## Meeting Objective

To formally launch the BSc Computing Science project, lay down the governance
framework, clarify each team member's role, agree how the group would
communicate throughout the year, and set a shared project vision that aligns
with the programme's academic requirements.

---

## Review of Previous Actions

This was our very first meeting, so there were no previous action items to
review. Before coming together, everyone had already read through the
university project brief and the module handbook, so we were ready to hit the
ground running.

---

## Detailed Discussion and Proceedings

### 1.1 Welcome and Programme Context

Sijan Bhandari opened the meeting warmly and confirmed that the project is
registered under the BSc Computing Science programme at Coventry University
Dagenham. The group took some time to walk through the module leader's brief,
which set out expectations around independent research, critical thinking, and
the standard of work required for the final deliverables. Everyone acknowledged
how significant the dissertation component is and agreed to treat it with the
seriousness it deserves.

### 1.2 Team Roles and Governance Structure

Roles were agreed and formally assigned: Sijan Bhandari stepped up as Project
Lead and Chairperson; Johns Jimee took on the responsibility of Secretary,
Technical Analyst, and QA Engineer, overseeing documentation, technical
reporting, testing and defect management; Abdullah Badhurdeen became our Lead
Developer, responsible for leading the technical build and implementation; Aditya Bhandari took the Systems Architect role,
handling design, planning, and resource coordination. Sandesh Shah and Nabin
Sunar joined as Project Associates and were fully brought on board from this
meeting onwards, with all documentation shared with them immediately.

### 1.3 Project Vision and Research Questions

Working collaboratively, the team shaped the project's primary research
question: exploring how modern game development frameworks support real-time
interactive systems at undergraduate educational levels. We also drafted
secondary questions covering performance benchmarking, user experience
evaluation, and comparative platform analysis. These questions will anchor
everything we do in the literature review chapter.

### 1.4 Communication and Collaboration Channels

The group agreed on a clear communication setup: Microsoft Teams for formal
scheduled meetings, WhatsApp for quick day-to-day coordination, and a shared
OneDrive folder for version-controlled documents. Meetings will be held
fortnightly, with extra sessions called if needed. Everyone committed to
handling project data responsibly and in line with GDPR obligations.

### 1.5 Ethical Considerations

We reviewed the university's ethics framework together. Since the project
includes user testing, the group agreed to prepare an ethics application form
covering informed consent, anonymisation of participant data, and full
compliance with the BSc Ethics Protocol. Johns Jimee volunteered to draft the
ethics submission before Meeting 3.

---

## Academic and Research Considerations

The project got underway drawing on established project management principles —
particularly PRINCE2 and Agile Scrum — alongside Saunders et al. (2019)
_Research Methods for Business Students_ to guide our research design. Academic
integrity, proper citation practices, and systematic evidence gathering were
emphasised as non-negotiable from day one.

---

## Risk Assessment

| Risk Description                                                          | Likelihood | Impact | Mitigation Strategy                                                                       |
| ------------------------------------------------------------------------- | ---------- | ------ | ----------------------------------------------------------------------------------------- |
| Team member unavailability due to competing academic commitments          | Medium     | High   | Cross-train roles and keep shared documentation so any member can step in at short notice |
| Unclear scope causing the project to drift beyond its original boundaries | High       | High   | Formalise a scope document in Meeting 2 and agree a change control process                |
| Communication breakdown across the team                                   | Low        | Medium | Weekly Teams check-in; WhatsApp group for anything urgent                                 |
| Ethics approval delays holding up the user testing phase                  | Medium     | High   | Submit the ethics form by Week 3 and build a buffer into the Gantt chart                  |

---

## Decisions Approved

1. Team structure, roles, and responsibilities formally agreed by all members.
2. Communication channels and meeting frequency confirmed.
3. Research questions and project vision approved.
4. Ethics application to be submitted before Meeting 3.
5. Project repository to be set up on university OneDrive by Johns Jimee within five working days.

---

## Action Plan

| Action Item                                                             | Owner               | Deadline    | Priority | Status |
| ----------------------------------------------------------------------- | ------------------- | ----------- | -------- | ------ |
| Draft the Project Charter and scope document                            | Sijan Bhandari      | 19 May 2026 | Critical | Open   |
| Set up the shared Github repository with a clear folder structure       | Abdullah Badhurdeen | 17 May 2026 | High     | Open   |
| Draft the Ethics Application Form                                       | Johns Jimee         | 22 May 2026 | High     | Open   |
| Begin researching game development frameworks for the literature review | Abdullah Badhurdeen | 5 June 2026 | High     | Open   |
| Prepare a draft Gantt Chart and project timeline                        | Aditya Bhandari     | 19 May 2026 | High     | Open   |
| Circulate meeting minutes to all team members                           | Johns Jimee         | 14 May 2026 | Medium   | Open   |

---

## Expected Deliverables

- Project Charter (v1.0)
- Draft Gantt Chart covering meetings and key submission deadlines
- Shared OneDrive repository (folder structure confirmed)
- Draft Ethics Application Form
- Research question register

---

## Meeting Outcome and Conclusion

Meeting 1 gave us a strong foundation. Everyone left with a clear sense of
purpose, agreed roles, and an understanding of what needs to happen next. The
project charter, timeline, and ethics submission are the immediate priorities
before we can move forward. The Chair closed the meeting at 12:00.

---

| Signatory   | Name           | Date        |
| ----------- | -------------- | ----------- |
| Chairperson | Sijan Bhandari | 12 May 2026 |
| Secretary   | Johns Jimee    | 12 May 2026 |

**Next Meeting:** 13 May 2026

_Document Version: v1.0 — Confidential_
